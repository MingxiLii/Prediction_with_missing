...
def masked_mae_cal(inputs, target, mask):
    ...
    return torch.sum(torch.abs(inputs - target) * mask) / (torch.sum(mask) + 1e-7)


def masked_mse_cal(inputs, target, mask):
    """calculate Mean Square Error"""
    return torch.sum(torch.square(inputs - target) * mask) / (torch.sum(mask) + 1e-7)
...
def masked_mre_cal(inputs, target, mask):
    ...
    return torch.sum(torch.abs(inputs - target) * mask) / (
        torch.sum(torch.abs(mask)) + 1e-7
    )

def masked_mape_cal(inputs, target, mask):
    percentage_errors = torch.abs((inputs - target) / (target+1e-9)) * 100 * mask

    # Calculate the sum of the masked errors and the number of masked elements
    sum_masked_errors = torch.sum(percentage_errors)
    num_masked_elements = torch.sum(mask)

    mean_masked_mape = sum_masked_errors / (num_masked_elements + 1e-5)

    return mean_masked_mape
