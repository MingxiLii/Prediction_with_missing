import numpy as np
from attnGN import selfAttenGN, load_metr_LA,load_PEMSD8,load_seattle,load_PEMS,ablation_1
import torch
...
parser.add_argument('--missRate', type=float, default=0.4)
...
wandb.init(project="traffic_attention_GN_compare", config=args)
if args.dataset=="LA":
...
for epoch in range(epochNum):
    ...
    x_masks = [torch.as_tensor(train_X_mask[step_t, :, :], dtype=torch.float32, device=device).to(device) for step_t in random_selected_idxs]
    x_holdouts = [torch.as_tensor(ori_X[step_t, :, :], dtype=torch.float32, device=device).to(device) for step_t in random_selected_idxs]
    indicates_masks = [torch.as_tensor(train_indicates_mask[step_t, :, :], dtype=torch.float32, device=device).to(device) for step_t in random_selected_idxs]
    input_graphs = [
        Data(x=torch.as_tensor(node_attr[step_t, :, :], dtype=torch.float32, device=device),
             ...
             y=torch.as_tensor(float(step_t)/(tr_len+test_len), dtype=torch.float32, device=device).unsqueeze(0).to(device)
             ) for step_t in random_selected_idxs]
    output_tensors, reconsLoss,imputeLoss,_ = model(input_graphs, num_processing_steps,x_masks,x_holdouts,indicates_masks,"train")
    ##prediction loss
    ...
    loss = args.lossweight* sum(loss_seq) / len(loss_seq)+ (1-args.lossweight)*(imputeLoss+ reconsLoss)
    optimizer.zero_grad()
...
with torch.no_grad():
    ...
    for tt in range(0, test_len - num_processing_steps):
        ...
        x_masks = [torch.as_tensor(test_X_mask[tt + step_t, :, : ], dtype=torch.float32, device=device).to(device) for step_t in range(num_processing_steps)]
        x_holdouts = [torch.as_tensor(ori_te_X[tt + step_t, :, :], dtype=torch.float32, device=device).to(device) for step_t in range(num_processing_steps)]
        indicates_masks = [
        ...
        input_graphs = [
            Data(x=torch.as_tensor(node_attr[tt + step_t, :, :], dtype=torch.float32, device=device),
                 edge_index=torch.as_tensor(edge_index, device=device).to(device),
                 edge_attr=torch.as_tensor(edge_attr, dtype=torch.float32, device=device).to(device),
                 y=torch.as_tensor(float(tt+step_t)/(tr_len+test_len), dtype=torch.float32, device=device).unsqueeze(0).to(device))
            for step_t in range(num_processing_steps)]

        output_tensors, _, _, imputed = model(input_graphs, num_processing_steps,x_masks,x_holdouts,indicates_masks,"test")
        ...
        imputed_mae= masked_mae_cal(torch.as_tensor(scaler.inverse_transform(imputed[0].squeeze(2).cpu().numpy().reshape(-1, 1))).reshape(ori_te_X.shape[1],-1),torch.as_tensor(scaler.inverse_transform(np.nan_to_num(ori_te_X[tt, :, :]).reshape(-1, 1))).reshape(ori_te_X.shape[1],-1),indicates_masks[0].cpu())
        imputed_rmse = masked_rmse_cal(torch.as_tensor(scaler.inverse_transform(imputed[0].squeeze(2).cpu().numpy().reshape(-1, 1))).reshape(ori_te_X.shape[1],-1),torch.as_tensor(scaler.inverse_transform(np.nan_to_num(ori_te_X[tt, :, :]).reshape(-1, 1))).reshape(ori_te_X.shape[1],-1),indicates_masks[0].cpu())
        imputed_mre = masked_mre_cal(torch.as_tensor(scaler.inverse_transform(imputed[0].squeeze(2).cpu().numpy().reshape(-1, 1))).reshape(ori_te_X.shape[1],-1),torch.as_tensor(scaler.inverse_transform(np.nan_to_num(ori_te_X[tt, :, :]).reshape(-1, 1))).reshape(ori_te_X.shape[1],-1), indicates_masks[0].cpu())
        ...
        # predict_plot.append(np.squeeze(scaler.inverse_transform(output_tensors[0].cpu().numpy().reshape(-1,1)), axis=1).reshape(-1,3))
        # true_predict_plot.append(np.squeeze(scaler.inverse_transform(test_y[tt].reshape(-1,1)),axis=1).reshape(-1,3))
